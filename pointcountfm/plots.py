...
# ──────────────────────────────────────────────────────────────────────────────
CONFIG = {
    # Input H5 file (ground truth showers)
    "input_file": (
    ...
    # ML output H5 files to compare: list of {path, label} dicts.
    # Each file must contain a "num_points_per_layer" dataset written by
    # inference_cond_file.py (shape: [N, num_layers], int32).
    "ml_files": [
        # {
        #     "path": (
        #         "/n/home04/hhanif/AllShowers/results"
        #         "/20260402_235544_Electron-PointCountFM/output.h5"
        #     ),
        #     "label": "concat squash",
        # },
        # {
        #     "path": (
        #         "/n/home04/hhanif/AllShowers/results"
        #         "/20260402_234941_Electron-PointCountFM/output.h5"
        #     ),
        #     "label": "fully connected",
        # },
        {
            "path": "/n/home04/hhanif/TAMBO-opt/results/20260416_050139_Electron-PointCountFM/output_00.h5",
    ...
    # Must match the range used when running inference_cond_file.py.
    "pool_stop": 500,
    ...
    # Name of the variable-length showers dataset inside the input H5 file.
    "showers_dset": "showers",

    # Dataset name inside each ml_file for the ML predictions.
    "ml_dset": "observables/num_points_per_layer",
...
def main():
    ...
    print(f"ML files to compare ({len(ml_specs)}):")
    for path, label in ml_specs:
        print(f"  [{label}]  {path}")

    # ── Resolve pool range ────────────────────────────────────────────────────
    file_len = showerdata.get_file_length(input_file)
    pool_end = file_len if pool_stop == -1 else min(pool_stop, file_len)
    print(f"\nPool: showers [0, {pool_end})  (file has {file_len} showers)")

    # ── Load conditioning data for the whole pool ─────────────────────────────
    print("Loading conditioning data for pool ...")
    t0 = time.time()
    cond_data = showerdata.load_inc_particles(input_file, start=0, stop=pool_end)
    ...
    # These are pool-relative indices (0-based within [0, pool_end)).
    sampled_pool:   dict[int, np.ndarray] = {}
    sampled_global: dict[int, np.ndarray] = {}
    ...
    for pdg in pdg_codes:
        ...
        chosen = rng.choice(pool_idx, size=min(showers_per_class, len(pool_idx)),
                            replace=False)
        chosen.sort()
        sampled_pool[pdg]   = chosen          # index into pool / ml_file rows
        sampled_global[pdg] = chosen          # pool starts at 0, so same here
    ...
    # ── Load ML predictions from H5 files ────────────────────────────────────
    all_ml: list[np.ndarray] = []
    all_labels: list[str]    = []

    for path, label in ml_specs:
        print(f"\nLoading ML predictions [{label}] from {path} ...")
        t0 = time.time()
        with h5py.File(path, "r") as hf:
            if ml_dset not in hf:
                raise KeyError(
                    f"Dataset '{ml_dset}' not found in {path}. "
                    f"Top-level keys: {list(hf.keys())}"
                )
            ml_full = hf[ml_dset][:]          # shape: [N_file, num_layers]

        if ml_full.ndim != 2 or ml_full.shape[1] != num_layers:
            raise ValueError(
                f"[{label}] expected shape (N, {num_layers}), got {ml_full.shape}"
            )
        if ml_full.shape[0] < pool_end:
            raise ValueError(
                f"[{label}] file has {ml_full.shape[0]} rows but pool_end={pool_end}. "
                "Re-run inference with a larger output or reduce pool_stop."
            )

        ml_sampled = ml_full[all_sampled_pool].astype(np.int32)
        print(f"  Loaded {ml_full.shape[0]} rows, sampled {n_total}  "
              f"({(time.time() - t0) * 1000.0:.1f} ms)")

        all_ml.append(ml_sampled)
    ...
    for pdg in pdg_codes:
        ...
        _save_average_plot(gt_all[sl], [ml[sl] for ml in all_ml], all_labels,
                           num_layers, n_cls, f" [{pdg_name}]", avg_path)
